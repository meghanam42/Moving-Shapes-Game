...
public abstract class MovingShape {
    ...
    public void setPath(int pathID) {
        switch (pathID) {
			...
			case 1 : {//case 1 to set to horizontal path
	...
	/*
     *  ===============================================================================
     *  HorizontalPath : A path that goes horizontally
     *  ===============================================================================
     */
    public class HorizontalPath extends MovingPath {
		private double amplitude = 0,step= 0,deltaX=0, deltaY = 0;// setting these variables to zero.
		
		public HorizontalPath(){//constructor to initialise values for constructor
			amplitude = Math.random() * 10;//amplitude
			step = 100.0;//the step value to increment the deltaY value
			deltaY = 0;//set the deltaY value to zero initially
			deltaX = 10;//set the deltaX value to 10.
			//this is basically the reverse of the FallingPath constructor where the deltaX value was set to zero while deltaY was set to 5.
		}
		public void move(){
			deltaY += step;//increment deltaY value
			topLeft.y = (int) Math.round(topLeft.y + amplitude * Math.sin(deltaY));//set new topLeft.y value
			topLeft.x += deltaX;//increment topLeft.x by deltaX done in a similar fashion to how deltaY was incremented in the FallingPath class
			if (topLeft.x > marginWidth) // if it reaches the right side of the frame, then start again from the left side of the frame.
               topLeft.x = 0;//and reset the topLeft.x value to zero.
		}
	}
}
